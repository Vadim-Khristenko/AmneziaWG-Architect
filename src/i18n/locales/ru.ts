...
export const ru = {
  ...
  "landing.lede":
    "Инструмент собирает параметры обфускации и объясняет каждое число. Не «сгенерировать и надеяться», а чертёж: видно, из чего состоит пакет и почему он выглядит именно так.",
  "landing.sheet": "Лист",
  ...
  "landing.hero.cta": "Открыть генератор AmneziaWG",
  "landing.hero.second": "Что это вообще такое",

  "landing.tools.title": "Два движка",
  "landing.tools.lede":
    "Один метод, два протокола. Параметры считаются в браузере и никуда не отправляются — ни здесь, ни там.",

  "landing.awg.name": "AmneziaWG",
  "landing.awg.desc":
    "Junk-поезда, диапазоны заголовков, размерное дополнение и подписи CPS. Одиннадцать профилей мимикрии, собранных по RFC, а не на глаз. Версии 1.0–3.0, десять клиентов со своими пределами.",
  "landing.awg.status": "Работает",
  "landing.awg.go": "К генератору",

  "landing.xray.name": "XRay / REALITY",
  "landing.xray.desc":
    "REALITY, DSL шифрования VLESS, транспорты XHTTP, отпечатки браузеров и FinalMask. Движок написан и покрыт тестами против выпущенных ядер; интерфейс к нему ещё собирается.",
  "landing.xray.status": "Движок готов, интерфейс собирается",
  "landing.xray.go": "Что уже есть",

  "landing.profiles.title": "Под что можно маскироваться",
  "landing.profiles.lede":
    "Каждый профиль собран из текста стандарта: длины полей, порядок, варианты — так, чтобы разбор сторонним стеком не спотыкался.",

  "landing.trust.title": "Почему этому можно верить",
  "landing.trust.local.title": "Некуда утекать",
  "landing.trust.local.desc":
    "Нет сервера. Ключи и параметры считаются в этой вкладке и остаются в ней — забирать их неоткуда и некому.",
  "landing.trust.refuse.title": "Отказывается от плохого конфига",
  "landing.trust.refuse.desc":
    "Инструмент знает правила: S1–S4 не меньше 12 при защите заголовков, диапазоны H1–H4 не пересекаются, у каждого клиента свой потолок. Что не проходит проверку — не выдаётся.",
  "landing.trust.open.title": "Проверяется, а не декларируется",
  "landing.trust.open.desc":
    "Профили мимикрии разбираются обратно как настоящие пакеты, параметры прогоняются десятками тысяч на инвариантах. Так нашлись две ошибки в длине QUIC, которые чтение кода не увидело.",

  "landing.more.title": "Что ещё есть",
  "landing.more.mergekeys":
    "Обновить обфускацию в существующем vpn:// ключе или собрать несколько ключей в один контейнер.",
  "landing.more.simulator":
    "Показать по шагам, как выглядит рукопожатие с выбранными параметрами.",
  "landing.more.faq": "Сорок четыре ответа: что делает каждый параметр и что проверять, когда туннель не поднимается.",
