...
export function genCfg(input: GeneratorInput): AWGConfig {
  ...
  // Важно: S4 официально ограничен 32 байтами. Большее значение отклоняется
  // ядром AmneziaWG и всеми официальными клиентами.
  let s4 = rnd(1, 32); // Data: 1-32 (жёсткий лимит протокола)

  // S3 extreme mode: расширяем диапазон при включённой опции.
  // S4 НЕ расширяется — превышение 32 ломает совместимость.
  if (useExtremeMax) {
