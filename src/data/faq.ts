...
export const FAQ_ENTRIES: FaqEntry[] = [
  ...
  },
  {
    ...
    answer: {
      ru: "Потому что nonce шифра нигде не передаётся отдельно — он берётся из первых 12 байт того самого случайного паддинга, который задают `S1–S4`. Это видно в коде отправки: буфер паддинга режется на 12 байт и используется как nonce.\n\nЕсли паддинг короче, срез уходит за его границу в тело сообщения. В Go это не вызывает паники, потому что срез остаётся в пределах ёмкости буфера, — и **именно поэтому проблема опасна: ошибки не будет, шифр просто тихо начнёт работать на предсказуемом nonce**.\n\nПоэтому генератор поднимает все четыре S до 12 байт, а валидатор отклоняет конфиги, где это нарушено.",
      en: "Because the cipher nonce is never transmitted separately — it is taken from the first 12 bytes of the same random padding that `S1–S4` define. You can see it in the [send path](https://github.com/amnezia-vpn/amneziawg-go/blob/master/device/send.go): the padding buffer is sliced to 12 bytes and used as the nonce.\n\nIf the padding is shorter, that slice runs past its end into the message body. In Go this raises no panic, because the slice stays within the buffer's capacity — which is exactly what makes it dangerous: **there is no error, the cipher just quietly starts running on a predictable nonce**.\n\nSo the generator raises all four S values to 12 bytes and the validator rejects configs that break it.",
    },
