...
export const TIMELINE: TimelineEntry[] = [
  ...
  },
  {
    ...
    color: "green",
    desc: {
      ru: "Панель параметров при выбранной 3.0 рисовала форму 1.x: пропадали S3/S4, а H1–H4 показывались одним числом вместо диапазона — при том что сам .conf был верным. Причина оказалась не в опечатке: «современную» версию каждый из шести файлов определял своей парой литералов, и один из них про 3.0 не знал. Теперь возможности версии объявлены один раз в generator/versions.ts, и генератор, рендер, симулятор, валидатор, гайд по полям и вкладки читают их оттуда — AmneziaWG 4.0 добавляется одной записью, а неизвестная версия отрисовывается по самой полной форме вместо пустой панели. Заодно S3/S4 перестали генерироваться там, где версия их не использует: раньше они создавались всегда и лишь прятались рендером. Разобрано, какие параметры обязаны совпадать. Приёмная сторона опознаёт пакет своими S и H, поэтому совпадать обязаны S1–S4, H1–H4 и HeaderProtectionKey; Jc, Jmin, Jmax, цепочка I1–I5 и ContentPaddingAddition — клиентские, у каждого устройства могут быть свои, и разные значения лучше одинаковых: одинаковый у сотни клиентов мусорный поезд даёт DPI готовый шаблон. Прежняя формулировка «все параметры 3.0 должны совпадать» была неверной и жила в трёх ответах FAQ, в README релизного архива и в шапке каждого конфига из awg-gen.sh. FAQ вырос с 33 до 44 ответов. Длинные имена 3.0 больше не сливаются: у них был uppercase, стиравший границы слов. История генераций пишется сразу, а не через таймер, и открывается под своей кнопкой. Релизный пайплайн наконец читает сообщение аннотированного тега — раньше релиз описывался только списком коммитов — и больше не советует устаревших команд запуска.",
      en: "With 3.0 selected, the parameter panel rendered a 1.x shape: S3/S4 disappeared and H1–H4 showed a single value instead of a range, even though the .conf underneath was correct. The cause was not a typo: each of six files decided what a modern version was with its own pair of literals, and one of them had never heard of 3.0. Version capabilities are now declared once in generator/versions.ts, and the generator, renderer, simulator, validator, field guide and version tabs all read from it — AmneziaWG 4.0 becomes a single entry, and an unknown version renders with the richest shape instead of a blank panel. S3/S4 also stopped being generated for versions that do not use them; they used to be drawn every time and merely hidden at render. Which parameters must match is now settled. The receiving side identifies a packet using its own S and H values, so S1–S4, H1–H4 and HeaderProtectionKey have to be identical; Jc, Jmin, Jmax, the I1–I5 chain and ContentPaddingAddition are client-side and may differ per device — and varied values beat identical ones, since one junk train shared by a hundred clients hands DPI a template. The old claim that every 3.0 parameter must match was wrong and had spread to three FAQ answers, the release archive README and the header of every config awg-gen.sh emits. The FAQ grew from 33 answers to 44. Long 3.0 names no longer run together: they were uppercased, which erased every word boundary. Generation history is written immediately rather than behind a timer, and opens under its own button. The release pipeline finally reads the annotated tag message — releases used to be described by a list of commits alone — and no longer suggests stale ways to run the project.",
    },
  },
];
